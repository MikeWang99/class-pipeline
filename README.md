...
| 课后 | 自动归档文字稿；转写完成后再确认真正发生的课次，并强制读取当前档案、本节课、历史课次和备课上下文，建立证据包后才生成家长反馈、更新同一学生档案并完成教师教学优化复盘 | 后台自动 + AI 周期任务 |
...
课程身份不再在开课时按“最近日历开始时间”永久锁定。录音开始时只缓存当天候选；转写完成后，系统用本节课真正有意义的对话时间区间与日历 start/end 区间比较重叠度。只有唯一高置信候选才自动绑定，歧义时保持“待AI识别学生”，绝不自动更新任何学生档案。反馈阶段会先生成 `feedback_context.json` 与 `feedback_evidence.json`：AI 必须完整读取当前学生档案、本节完整文字稿，以及存在的上次反馈、上次课堂、当前/下一次备课；“当前待加强方向”必须有本节课直接证据。验证通过后 `scripts/trigger_postclass_ai.sh` 才完成家长反馈、档案更新和教学优化复盘。这不是定时轮询；每天 10:00 的 Codex 自动任务仅作为失败重试与次日备课入口。
